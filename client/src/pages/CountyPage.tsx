...
export default function CountyPage() {
  ...
  const county = territory?.tier1Counties.find(c => c.slug === countySlug);
  ...
  const path = `/${stateSlug}/${countySlug}/precision-agriculture`;
  useSEO({ path });

  const schemas = [
    generateServiceSchema({
      name: `Precision Agriculture Services in ${county.county} County, ${territory.name}`,
      description: `RTK guidance, planter technology, and steering systems for ${county.county} County farmers`,
      url: path,
      areaServed: [county.county + " County, " + territory.name]
    }),
          ...
          <div className="container mx-auto px-4">
            ...
            <div className="flex flex-wrap gap-3">
              {county.cities.map(city => (
                ...
                >
                  <Button variant="outline" className="rounded-none border-green-600 text-green-600 hover:bg-green-600 hover:text-white" data-testid={`button-city-${city.slug}`}>
